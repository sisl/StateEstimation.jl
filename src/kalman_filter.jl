...
begin
	...
	T(s,a) = [clamp.(s .+ a, 𝒮ₘᵢₙ, 𝒮ₘₐₓ)] # deterministic transition
	O(a, s′) = MvNormal(𝒪.μ + s′, 𝒪.Σ*abs.(a)) # observation distribution
end;

# ╔═╡ 0ac657c9-ead6-4bbb-a6cc-a004c8563ac4
md"""
# Standard KF visualization
"""
...
# ╔═╡ da750507-dca0-4508-b4ad-a4901defed79
md"""
# Sigma point samples visualization
Play around with the parameter sliders!
"""

# ╔═╡ d0253f60-0eba-11eb-0283-e5b6e8c14ffe
md"""
 $$\Sigma_{11}$$ = $(@bind Σ₁₁ Slider(1:5, default=1, show_value=true)) | 
$$\Sigma_{12}$$ = $(@bind Σ₁₂ Slider(-5:5, default=0, show_value=true))

 $$\Sigma_{21}$$ = $(@bind Σ₂₁ Slider(-5:5, default=0, show_value=true)) |
$$\Sigma_{22}$$ = $(@bind Σ₂₂ Slider(1:5, default=1, show_value=true)) 

 $$\lambda$$ = $(@bind λₑₓ Slider(-1:10, default=2, show_value=true))
"""
...
cross_cov(μₚ,μₒ,wₛ,Sₒ,Sₒ′) = sum(w*(s - μₚ)*(s′ - μₒ)' for (w,s,s′) in zip(wₛ,Sₒ,Sₒ′))

# ╔═╡ d9d1a4c0-fc5b-11ea-0d6c-ab55c2b33d19
function ukf_update!(b::UnscentedKalmanFilter, 𝒫::POMDPᵘᵏᶠ, o, μₚ, Σₚ, wₛ)
	# 1. Update observation model
	(μₒ, Σₒ, Sₒ, Sₒ′) = ukf_update_observation(b, 𝒫, μₚ, Σₚ, wₛ)

	# 2. Calculate the cross covariance matrix
	Σₚₒ = cross_cov(μₚ, μₒ, wₛ, Sₒ, Sₒ′)

	# 3. Update belief
	K = Σₚₒ / Σₒ           # Kalman gain
	b.μᵦ = μₚ + K*(o - μₒ) # updated mean
	b.Σᵦ = Σₚ - K*Σₒ*K'    # updated covariance
end
...
function plot_kalman_filter(belief, true_state, iter, action)
	plot(legend=false, axisratio=:equal, dpi=600)
...
md"# Standard KF: Simulation and testing"
...
julia_version = "1.9.0"
manifest_format = "2.0"
project_hash = "de76c9ff508e881d4c16216d8bc91480f6f1dabb"
