...
# ╔═╡ 5b7f86a6-34a6-44fe-bb89-945aaff2b2bb
using Test
...
# ╔═╡ 7f8af43d-4ec5-464a-90c8-add21464c5be
md"""
# Code (Matrices)
"""

# ╔═╡ 5a53d8f1-6ddd-42b9-8c48-f26e4889677c
md"""
We also define _particle filtering_ for our belief $\mathbf{s}$ represented by a `Matrix` (instead of a `Vector`).
"""

# ╔═╡ 30e8fc3e-26af-4386-bc45-f56e6bbeaeb0
md"""
...
function particle_filter(𝐬::Vector, 𝒫::POMDP, a, o)
	(T, O) = (𝒫.T, 𝒫.O)
	𝐬′ = rand.(T.(𝐬, a))                # next states
	𝐰 = O.(a, 𝐬′, o)                    # weights
...
function particle_filter(𝐬::Matrix, 𝒫::POMDP, a, o)
	(T, O) = (𝒫.T, 𝒫.O)
	𝐬′ = mapslices(s->rand(T(s, a)), 𝐬; dims=1)   # next states
	𝐰 = mapslices(s′->O(a, s′, o), 𝐬′; dims=1)    # weights
	𝐰ₙ = mapslices(w->normalize(w, 1), 𝐰; dims=2)
	if isnan(sum(𝐰ₙ))
		fill!(𝐰ₙ, 1/length(𝐰ₙ))
	end
	𝒞 = Categorical(vec(𝐰ₙ))
	return 𝐬′[:, rand(𝒞, size(𝐬′, 2))]            # sample with normalized weights
end
...
begin
	...
	𝒜 = Normal(0, 1)
	𝒪 = Uniform(-10, 10)
	T(s,a) = Normal(clamp(s+a, minimum(𝒮), maximum(𝒮)), abs(a))
	O(a, s′) = Normal(s′, abs(a))
	O(a, s′,o) = pdf(O(a, s′), o)
	𝒫 = POMDP(𝒮, 𝒜, 𝒪, T, O)
end

# ╔═╡ 8107bcc4-30ab-47b6-b97a-995b0fd97015
md"""
...
# ╔═╡ a6678d45-9150-4940-aec6-5eba2db87486
md"""
# Step 2 (next states)
"""
...
# ╔═╡ 5b6da9c5-68fa-427b-b0ed-083bb7eebf46
md"""
# Step 3 (weights)
"""
...
# ╔═╡ a7c5dd9c-df09-446b-ac75-5de691ab5f18
md"""
...
# ╔═╡ a692b9b8-6ba0-4d0e-a1e2-28b824c586c9
md"""
...
# ╔═╡ b7e0c385-c5c5-48c3-b85a-cbb28ca113e1
md"""
# Step (function)
"""

# ╔═╡ a89bbc40-fb77-11ea-3a1b-7197afa0c9b0
function step(𝒫, 𝐛, 𝒜, s, a, o, T, O)
	a = rand(𝒜)
	s′ = rand(T(s, a))
	o = rand(O(a, s′))
...
# ╔═╡ 3e66ecda-417d-4d97-853e-b560fc3b8f88
md"""
# Visualization
"""
...
begin
    ...
    T2(s,a) = MvNormal(clamp.(s .+ a, 𝒮ₘᵢₙ, 𝒮ₘₐₓ), I*abs.(a))

	O2(a, s′) = MvNormal(s′, I*abs.(a))
    O2(a, s′,o) = pdf(O2(a, s′), o)
    𝒫2 = POMDP(𝒮2, 𝒜2, 𝒪2, T2, O2)
end;

# ╔═╡ 2c1b19e8-f12b-449c-b201-94352043de33
md"""
# Visualization
"""
...
# ╔═╡ fa4691ec-3f0f-4b6d-bc5c-ad8fca138fdd
begin
	import Logging
...
with_terminal() do
	@testset begin
		...
		global a = 1e-10
		for i in 1:t
			(belief, s, a, o) = step(𝒫, belief, 𝒜, s, a, o, T, O)
			test_filter(belief, s) # test that belief covers some σ
		end
	end
end
...
# ╔═╡ 3da3e434-e9e0-452a-a5d7-7dfe3c125d60
md"""
# Walk 2D Simulation
"""
...
with_terminal() do
	...
	for i in 1:t2
		(belief2, s2, a2, o2) = step(𝒫2, belief2, 𝒜2, s2, a2, o2, T2, O2)
	end
	@show s2
end
...
# ╔═╡ cb8328ed-367e-4ab2-8855-bf9c5cfce30d
md"""
---
"""
